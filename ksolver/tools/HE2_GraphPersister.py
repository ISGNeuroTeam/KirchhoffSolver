...
import ksolver.fluids.HE2_Fluid as he2_fluid

class_aliases = ["OilPipeSegment", "WaterPipeSegment", "OilPipe", "WaterPipe"]
class_aliases += ["DummyWater", "DummyOil", "OilWater", "Plast", "WellPump"]

classes = [
    HE2_Pipe.HE2_OilPipeSegment,
    HE2_Pipe.HE2_WaterPipeSegment,
    HE2_Pipe.HE2_OilPipe,
    HE2_Pipe.HE2_WaterPipe,
]
classes += [
    he2_fluid.HE2_DummyWater,
    he2_fluid.gimme_dummy_BlackOil,
    he2_fluid.HE2_BlackOil,
    HE2_Plast,
    HE2_WellPump,
]
...
def dict_to_fluid(obj_dict):
    ...
    if class_alias == "DummyOil":
        init_kwargs = dict(
            daily_Q=obj_dict["Q_m3_day"], VolumeWater=obj_dict["volumewater_percent"]
        )
...
def test3():
    op = oil_params.dummy_oil_params(10, 50)
    fluid = he2_fluid.HE2_BlackOil(op)
    pipe = HE2_Pipe.HE2_OilPipe(
        [100, 200], [-10, 20], [0.3, 0.4], [1e-5, 1e-5], [fluid, fluid]
    )
    d = pipe_to_dict(pipe)
