...
import pandas as pd
from ksolver.tools.HE2_tools import make_oilupstream_graph_layout
...
def plot_some_subgraph(G, G_tree, G_chordes, solver, keys_to_plot):
    ...
    nx.draw_networkx_nodes(
        G, node_size=15, node_color="black", ax=ax, pos=pos, alpha=0.9
    )
    nx.draw_networkx_edges(
        G, edgelist=G_tree, pos=pos, width=1, ax=ax, edge_color="black"
    )
    nx.draw_networkx_edges(
        G,
        edgelist=G_chordes,
...
def plot_all(
    solver, keys_to_plot=("name", "P", "Q", "x", "dp/dx", "dP", "WC", "GOR", "pos")
):
    G = solver.graph
