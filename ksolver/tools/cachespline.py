...
def create_lazy_spline_cache_f_wrapper(
    f, dx=0.1, dy=0.1, half_nx=4, half_ny=4, kind="cubic", err_control_rate=0.001
):
    info = Counter()
    nx = 2 * half_nx
    ny = 2 * half_ny
    ...
    def f_caller(x, y):
        ...
        if not (x_knot_idx, y_knot_idx) in covered:
            info["cache_miss"] += 1
            surface_piece_spline = build_spline_surface_over_knot(
                x_knot_idx, y_knot_idx
            )
