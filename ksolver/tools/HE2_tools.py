...
from ksolver.graph.edges.HE2_Pipe import HE2_WaterPipe
...
CheckSolutionResults = namedtuple(
    "CheckSolutionResults",
    [
        "first_CL_resd",
        "second_CL_resd",
        "negative_P",
...
def generate_random_net_v0(
    ...
    sinks = {
        f"sink_{i}": vrtxs.HE2_Boundary_Vertex("Q", -sink_q[i]) for i in range(SNK)
    }

    nodes = {**p_nodes, **sources, **sinks, **juncs}
    mapping = dict(zip(range(len(nodes)), nodes.keys()))
    UDRG = nx.generators.random_graphs.gnm_random_graph(
        N, E, directed=False, seed=randseed
    )
...
def generate_random_net_v1(
    ...
    edgelist += [
        tuple(np.random.choice(range(N), 2, replace=False)) for i in range(E - (N - 1))
    ]
...
#     label_pos = (
#         {k: (pos[k][0] + shifts[k][0], pos[k][1] + shifts[k][1]) for k in pos}
...
#                 G,
#                 nodelist=list(set(nodelist) & g_nodes),
#                 node_size=node_size,
...
#                 alpha=0.3,
#             )
#             HE2_draw_node_labels(
#                 G,
#                 g_nodes,
#                 list(set(nodelist) & g_nodes),
#                 keys=["P_bar"] + ks,
#                 ax=ax,
#                 pos=label_pos,
#             )
...
#     if type(G) == nx.MultiDiGraph:
#         edge_labels = {
#             (u, v): f"{G[u][v][k]['obj'].result['x']:.2f}" for u, v, k in G.edges
#         }
#     else:
#         edge_labels = {(u, v): f"{G[u][v]['obj'].result['x']:.2f}" for u, v in edgs}
#
#     nx.draw_networkx_edge_labels(
#         G, pos=pos, edge_labels=edge_labels, font_size=9, alpha=0.3
#     )
#
...
def split_mass_flow_to_OWG(x, fluid: HE2_BlackOil):
    ...
    oil_ro, wat_ro, gas_ro = (
        oil_params.oildensity_kg_m3,
        oil_params.waterdensity_kg_m3,
        oil_params.gasdensity_kg_m3,
    )
...
def generate_superpositioned_colored_flows_graph(
    N=10, E=13, SRC=3, SNK=3, randseed=None
):
    return
    ...
    for n in sources:
        ...
        flow_value, flow_dict = nx.algorithms.flow.maximum_flow(
            scaffold, "SUPERSOURCE", "SUPERSINK"
        )
...
def split_input_df_to_pipes_and_boundaries(df):
    ...
    start_cols = [col + "_start" for col in bnd_cols2] + [
        "start_" + col for col in bnd_cols1
    ]
...
def print_wells_pressures(G, wells):
    ...
    for pad_well in wells:
        ...
        row_header = (
            "pad " + f" {pad}"[-2:] + " well " + f"  {well}"[-4:] + ", from plast:   "
        )
...
def print_solution(G):
    colorama.init()
    table_header = f' {"start":>33} {"P_bar":>8} {"Q kg/s":>8}   {"end":>33} {"P_bar":>8} {"Q kg/s":>8}    {"X kg/s":>7} {"WC %":>5}'
    print(table_header)
    for e in G.edges:
        if len(e) == 2:
            u, v = e
        ...
        u_obj = G.nodes[u]["obj"]
        v_obj = G.nodes[v]["obj"]
        x = obj.result.get("x", np.nan)
        wc = obj.result.get("WC", np.nan)
        p_u = u_obj.result.get("P_bar", np.nan)
        pu_str = f"{p_u:8.3f}"
        if p_u <= 1:
            pu_str = Back.RED + pu_str + Style.RESET_ALL

        p_v = v_obj.result.get("P_bar", np.nan)
        pv_str = f"{p_v:8.3f}"
        if p_v <= 1:
            pv_str = Back.RED + pv_str + Style.RESET_ALL

        q_u = u_obj.result.get("Q", 0)
        q_u_str = ""
        if abs(q_u) > 1e-5:
            q_u_str = f"{q_u:8.3f}"

        q_v = v_obj.result.get("Q", 0)
        q_v_str = ""
        if abs(q_v) > 1e-5:
            q_v_str = f"{q_v:8.3f}"

        row = f" {u:>33} {pu_str:>8} {q_u_str:>8}   {v:>33} {pv_str:>8} {q_v_str:>8}    {x:7.3f} {wc:5.1f}{Style.RESET_ALL}"
        print(row)


def cut_single_well_subgraph(G, pad_name, well, nodes=None, make_intake_boundary=False):
    ...
    if make_intake_boundary:
        fluid = node_objs[nodes[0]].fluid
        node_objs[nodes[2]] = vrtxs.HE2_Source_Vertex("P", 50, fluid, 20)
...
def iterate_edges(G):
    ...
    if type(G) == nx.MultiDiGraph:
        for u, v, k in G.edges:
            yield (u, v, k), G[u][v][k]['obj']

def dump_graph_results_to_dicts(G):
    n_rez = {node: G.nodes[node]['obj'].result for node in G.nodes}
    e_rez = {edge: obj.result for edge, obj in iterate_edges(G)}
    rez = dict(nodes=n_rez, edges=e_rez)
    return rez

def make_str_keyed_edges_dict(edges_results_dict):
    edges = edges_results_dict
    str_keyed_edges = {', '.join(map(str, k)): edges[k] for k in edges}
    return str_keyed_edges

def dump_graph_results_to_json_str(nodes, edges):
    str_keyed_edges = make_str_keyed_edges_dict(edges)
    dict_to_dump = dict(nodes=nodes, edges=str_keyed_edges)
    rez = json.dumps(dict_to_dump)
    return rez

def load_graph_results_from_json_str(json_str):
