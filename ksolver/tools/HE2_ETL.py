...
class HE2_ETL:
    ...
    def __init__(self, postgres_credentials, eva_credentials):
        self.pg_eng = create_engine(
            postgres_credentials,
            connect_args={"options": '-c search_path=dbo,public,"HE2"'},
        )
        ...
        process_list += [
            dict(
                kind="EVA",
                src="omds_well_wellop",
                dst="wellop_better_than_nothing",
        ...
        process_list += [
            dict(kind="xls", src="..\\data\\zak.xlsx", dst="pads_telemetry")
        ]
        process_list += [
            dict(
                kind="xls", src="..\\data\\TAYL-K1_K2_Out_PI.xlsx", dst="kns_telemetry"
            )
        ]
        process_list += [
            dict(kind="xls", src="..\\data\\Visio schema.xlsx", dst="visio_schema")
        ]
    ...
    def table_exists(self, table_name):
        ...
        try:
            probe_df = pd.read_sql(
                f"Select * from {table_name} limit 10", con=self.pg_eng
            )
    ...
    def move_data_from_eva_to_postgres(self, src, dst, **kwargs):
        ...
        df = pd.DataFrame(res)
        df.to_sql(
            name=dst, con=self.pg_eng, schema="HE2", if_exists="replace", index=False
        )
...
if __name__ == "__main__":
    ...
    timespan = (
        datetime.fromisoformat("2019-02-01 01:00"),
        datetime.fromisoformat("2019-02-01 01:15"),
    )

    pipeline = HE2_ETL(
        "postgresql://db_user:210106@localhost:5432/postgres",
        dict(host="192.168.4.65", port="80", user="am", password="12345678"),
    )
