from ksolver.io import HE2_DateframeWrapper as model
import scipy.optimize as scop
import numpy as np
import pandas as pd
from ksolver.solver.HE2_Solver import HE2_Solver
from ksolver.tools.HE2_schema_maker import (
    make_oilpipe_schema_from_OT_dataset,
    make_calc_df_oil,
)
from ksolver.tools.HE2_tools import print_solution, cut_single_well_subgraph
...
class HE2_PMNetwork_Model:
    ...
    def regularizator(self):
        ...
        terms = np.zeros(4)
        terms[0] = (
            np.linalg.norm((np.ones(N) - self.d_weights)) * self.reg_d_deviation_keff
        )
        terms[1] = (
            np.linalg.norm((np.ones(N) - self.r_weights)) * self.reg_r_deviation_keff
        )
    ...
    def fit_v0(self):
        ...
        bnds = list(
            zip(
                list(np.ones(self.row_count) * 0.35),
                list(np.ones(self.row_count) * 1.2),
            )
        )
        ...
        try:
            op_result = scop.minimize(
                target,
                x0,
                method=self.minimize_method,
                bounds=bnds,
                options=dict(nfev=100500),
            )
        except:
            op_result = scop.OptimizeResult(
                success=True, fun=self.best_y, x=self.best_x, nfev=self.it
            )
        return op_result
    ...
    def fit_v3(self):
        ...
        bnds = list(
            zip(
                list(np.ones(self.row_count) * 0.35),
                list(np.ones(self.row_count) * 1.0),
            )
        )
    ...
    def fit_v5(self):
        ...
        bnds = list(
            zip(
                list(np.ones(self.row_count) * 0.35),
                list(np.ones(self.row_count) * 1.1),
            )
        )
        x0 = np.array(
            [
...
class HE2_OilGatheringNetwork_Model:
    ...
    def gimme_calc_df(self, i):
        ...
        calc_df = make_calc_df_oil(original_df, self.commondata_folder)
    ...
    def gimme_graph(self, i):
        ...
        G, _, __ = make_oilpipe_schema_from_OT_dataset(
            df, self.commondata_folder, calc_df, ignore_Watercut=self.ignore_watercut
        )
        self.graphs[i] = G
        return self.graphs[i]
    ...
    def grab_fact(self):
        ...
        return dict(
            p_zab=p_zab, p_intake=p_intake, p_head=p_head, q_well=q_well, freq=freq
        )
        ...
        for pad in pad_wells_dict:
            ...
            for well in wells:
                ...
                for i in range(self.N):
                    ...
                    q_well[key][i] = (
                        86400 * obj.result["x"] / obj.result["liquid_density"]
                    )
    ...
    def plot_fact_and_results(
        self, keys_to_plot=("head", "intake", "bottom", "debit"), wells=(), pads=()
    ):
        ...
        for i, (pad, well) in enumerate(self.pad_well_list):
            ...
            if "head" in keys_to_plot:
                ax.plot(
                    self.fact["p_head"][(pad, well)],
                    self.result["p_head"][(pad, well)],
                    color=colors[i],
                    linewidth=1,
                    alpha=alpha,
                )
            if "intake" in keys_to_plot:
                ax.plot(
                    self.fact["p_intake"][(pad, well)],
                    self.result["p_intake"][(pad, well)],
                    color=colors[i],
                    linewidth=1,
                    alpha=alpha,
                )
            if "bottom" in keys_to_plot:
                ax.plot(
                    self.fact["p_zab"][(pad, well)],
                    self.result["p_zab"][(pad, well)],
                    color=colors[i],
                    linewidth=1,
                    alpha=alpha,
                )
            if "debit" in keys_to_plot:
                ax.plot(
                    self.fact["q_well"][(pad, well)],
                    self.result["q_well"][(pad, well)],
                    color=colors[i],
                    linewidth=1,
                    alpha=alpha,
                )
    ...
    def calc_well_score(self, pad, well):
        ...
        score = 0
        score += (
            abs(self.fact["p_head"][(pad, well)] - self.result["p_head"][(pad, well)])
            / head_scale
        )
        score += (
            abs(
        ...
        score += (
            abs(self.fact["p_zab"][(pad, well)] - self.result["p_zab"][(pad, well)])
            / bottom_scale
        )
        score += (
            abs(self.fact["q_well"][(pad, well)] - self.result["q_well"][(pad, well)])
            / debit_scale
        )
        return score
    ...
    def score_well(self, solver, well_G, pad, well, nodes):
        ...
        score += (
            np.linalg.norm(self.fact["p_zab"][(pad, well)] - rez_p_b) * weights["p_zab"]
        )
        return score

    def plot_single_well_chart(
        self,
        pad,
        well,
        well_res_before,
        ...
        ax = fig.add_subplot(1, 1, 1)
        ax.set_title(
            f"Pad {pad} well {well} y {op_result.fun:.3f}, prod {op_result.x[0]:.3f}  pump_keff {op_result.x[1]:.3f}"
        )
        ...
        ax.set_title(
            f"Pad {pad} well {well} WC {WC}% y {op_result.fun:.3f}, prod {op_result.x[0]:.3f}  pump_keff {op_result.x[1]:.3f}"
        )
        # ax.set_xlabel('time')
        ax.set_ylabel("P, Q")
        ...
        hndl1 = ax.plot(
            xs[mask],
            self.fact["p_intake"][(pad, well)][mask],
            color=colors[0],
            label="Fact: intake",
            **kwargs,
        )
        ...
        hndl3 = ax.plot(
            xs[mask],
            self.fact["q_well"][(pad, well)][mask],
            color=colors[2],
            label="Fact: debit",
            **kwargs,
        )
        ...
        hndl5 = ax.plot(
            xs,
            well_res_after["p_intake"],
            color=colors[0],
            label="Predict: intake",
            **kwargs,
        )
    ...
    def greed_optimization(self):
        ...
        for i, (pad, well, param) in enumerate(order):
            ...
            if i % 50 == 2:
                need_dump = True
            fitlog += [
                (i, (datetime.now() - start_time).seconds, self.last_it_count, best_y)
            ]
...
if __name__ == "__main__":
    model = HE2_OilGatheringNetwork_Model(
        commondata_folder="../../CommonData/", wells_folder="../../data/fit 22-06-2021/"
    )
