from ksolver.hydraulics.Formulas import get_dens_freegas
...
def two_phase_flow(P_bar, T_C, X_kg_sec, calc_params: oil_params, tubing_IntDiam=0):
    ...
    CurrentWaterViscosity = 0.00178 / (
        1 + 0.037 * (CurrentT - 273) + 0.00022 * (CurrentT - 273) ** 2
    )

    # Вспомогательные коэффициенты
    a1 = 1 + 0.0054 * (CurrentT - 303)
    a2 = (
        1 + np.log10(abs(CurrentP)) / (1 + np.log10(abs(SaturationPressure_MPa))) - 1
    )  # !!!!!!!!!!!
    ...
    SaturatedOilDensity = (
        SepOilDensity
        * (1 + 1.293e-3 * DissolvedGasDensity * GasFactor / (a1 * m))
        / OilVolumeCoeff
    )
    VolumeOil = 1 - VolumeWater
    # CurrentLiquidDensity = VolumeWater * PlastWaterDensity + VolumeOil * SaturatedOilDensity
    CurrentLiquidDensity = (
        VolumeWater * PlastWaterDensity + VolumeOil * SaturatedOilDensity
    )
    ...
    CurrentOilViscosity = (
        CurrentWaterViscosity if VolumeWater == 1 else CurrentOilViscosity
    )

    return Mishenko(
        oil_params=calc_params,
        CurrentP_MPa=CurrentP,
        CurrentT_K=CurrentT,
        VolumeWater_fraction=VolumeWater,
        Q_liq_m3_s=Q,
        g=g,
        SaturationPressure_MPa=Saturation_pressure,
        DissolvedGasAmount=1,
        FreeGasDensity_kg_m3=0,
        SaturatedOilDensity_kg_m3=SaturatedOilDensity,
        CurrentWaterDensity_kg_m3=PlastWaterDensity,
        CurrentOilViscosity_Pa_s=CurrentOilViscosity,
        CurrentWaterViscosity_Pa_s=CurrentWaterViscosity,
        RelativePressure=-100500,
        RelativeTemperature=-100500,
        CurrentFreeGasDensity_kg_m3=0,
        CurrentFreeGasViscosity_Pa_s=0,
        CurrentLiquidDensity_kg_m3=CurrentLiquidDensity,
        TensionOilGas=TensionOilGas,
        TensionWaterGas=TensionWaterGas,
        TensionOilWater=TensionOilWater,
        Q_gas_m3_s=0,
        Q_liq_and_gas_m3_s=Q,
...
def three_phase_flow(P_bar, T_C, X_kg_sec, calc_params):
    ...
    if CurrentP < 0:
        ...
        DissolvedGasDensity = (
            GasFactor
            * (a1 * m * GasDensity - FreeGasDensity * (FreeGasAmount / GasFactor))
            / DissolvedGasAmount
        )

    SaturatedOilDensity = (
        SepOilDensity
        * (1 + 1.293e-3 * DissolvedGasDensity * DissolvedGasAmount / (a1 * m))
        / OilVolumeCoeff
    )
    ...
    try:
        ...
        E = 0.025
    mu0 = (
        0.0101 * abs(CurrentT - 273) ** 1.8 - 1.07e-1 * (abs(Mg)) ** 0.5
    )  # !!!!!!!!!!!!!
    if CurrentP >= 5:
        CurrentFreeGasViscosity = (
            mu0 + 1.08e-4 / E * (np.exp(1.44 * r) - np.exp(-1.11 * r ** 1.86))
        ) * 1e-6
    else:
        CurrentFreeGasViscosity = (
            mu0
            + 1.08e-4 / E
            + (0.1023 + 0.23 * r + 0.0585 * r ** 2 - 0.0408 * r ** 3) / E
        ) * 1e-6

    # Плотность жидкости в текущих условиях
    CurrentLiquidDensity = (
        SaturatedOilDensity * (1 - VolumeWater) + CurrentWaterDensity * VolumeWater
    )
    # Поверхностное натяжение нефть - газ
    TensionOilGas = 10 ** -(1.58 + 0.05 * CurrentP) - 72e-6 * (CurrentT - 303)
    ...
    OWG_density = CurrentLiquidDensity + CurrentFreeGasDensity * FreeGasFactor * (
        1 - VolumeWater
    )
    ...
    VolumeGas = (
        Q_gas / (Q_gas + Q_liquid) if Q_owg != 0 else 0
    )  # if CurrentP < SaturationPressure_MPa else 0
    ...
    return Mishenko(
        ...
        g=g,
        SaturationPressure_MPa=Saturation_pressure,
        DissolvedGasAmount=DissolvedGasAmount,
        FreeGasDensity_kg_m3=FreeGasDensity,
        SaturatedOilDensity_kg_m3=SaturatedOilDensity,
        CurrentWaterDensity_kg_m3=CurrentWaterDensity,
        CurrentOilViscosity_Pa_s=CurrentOilViscosity,
        CurrentWaterViscosity_Pa_s=CurrentWaterViscosity,
        ...
        CurrentFreeGasViscosity_Pa_s=CurrentFreeGasViscosity,
        CurrentLiquidDensity_kg_m3=CurrentLiquidDensity,
        TensionOilGas=TensionOilGas,
        TensionWaterGas=TensionWaterGas,
        TensionOilWater=TensionOilWater,
        Q_gas_m3_s=Q_gas,
        Q_liq_and_gas_m3_s=Q_owg,
