...
def get_flow_structure_MB(Lw, Lg, Lm, angle, IntDiameter, Roughness):
    ...
    x1 = (
        np.log10(Lg)
        + 0.940
        + 0.074 * np.sin(np.radians(angle))
        - 0.855 * np.sin(np.radians(angle)) ** 2
        + 3.695 * Lm
    )
    ...
    x2 = (
        0.431
        - 3.003 * Lm
        - 1.138 * np.sin(np.radians(angle)) * np.log10(Lw)
        - 0.429 * np.sin(np.radians(angle)) * (np.log10(Lw)) ** 2
        + 1.132 * np.sin(np.radians(angle))
    )
    Lg_b_s = 10 ** x2
    ...
    z = (
        0.321
        - 0.017 * Lg
        - 4.267 * np.sin(np.radians(angle))
        - 2.972 * Lm
        - 0.033 * (np.log10(Lg) ** 2) ** 2
        - 3.925 * np.sin(np.radians(angle)) ** 2
    )
    Lw_s_t = 10 ** z
...
def get_coefs_MB(angle, IntDiameter, Roughness, flow):
    coefs = [
        (-0.380113, 0.129875, -0.119788, 2.343227, 0.475686, 0.288657),
