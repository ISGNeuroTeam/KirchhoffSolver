...
class Fluid:
    ...
    def _calc_pb_MPaa(
        self, number_cor
    ):  # TODO калибровку свойств делать внутри функций контейнеров, чтобы оставался выбор корреляций
        if number_cor == 0:
            return PVT.unf_pb_Standing_MPaa(
                self.rsb_m3m3, self.gamma_oil, self.gamma_gas, self.t_k
            )
        if number_cor == 1:
            return PVT.unf_pb_Valko_MPaa(
                self.rsb_m3m3, self.gamma_oil, self.gamma_gas, self.t_k
            )
        if number_cor == 2:  # TODO check cor
            return PVT.unf_pb_Glaso_MPaa(
                self.rsb_m3m3, self.t_res_k, self.gamma_oil, self.gamma_gas
            )
        if number_cor == 3:
            return PVT.calc_pb(
                self.pb_bar_for_rus_cor / 10,
                self.gamma_oil * 1000,
                self.rsb_m3m3,
                self.t_res_k,
                self.t_k,
    ...
    def _calc_rs_m3m3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_rs_Standing_m3m3(
                self.p_mpa,
                self.pb_mpa,
                self.rsb_m3m3,
                self.gamma_oil,
        ...
        if number_cor == 1:
            return PVT.unf_rs_Velarde_m3m3(
                self.p_mpa,
                self.pb_mpa,
                self.rsb_m3m3,
                self.gamma_oil,
        ...
        if number_cor == 2:
            return PVT.calc_gas_with_fsolve(
                self.t_k,
                self.gamma_oil * 1000,
                self.gamma_gas,
                self.p_mpa,
    ...
    def _calc_compr_oil_1mpa(self, number_cor):
        if number_cor == 0:
            return PVT.unf_compressibility_oil_VB_1Mpa(
                self.rs_m3m3, self.t_k, self.gamma_oil, self.p_mpa, self.gamma_gas
            )
        if number_cor == 1:
            return PVT.unf_compessibility_oil_VB_1MPa_vba(
                self.rsb_m3m3, self.t_k, self.gamma_oil, self.gamma_gas
            )

    def _calc_compr_oil_below_pb_1mpa(self, number_cor):
        if number_cor == 0:
            return PVT.unf_compressibility_saturated_oil_McCain_1Mpa(
                self.p_mpa, self.pb_mpa, self.t_k, self.gamma_oil, self.rsb_m3m3
            )
        if number_cor == 1:
            return PVT.unf_compessibility_oil_VB_1MPa_vba(
                self.rsb_m3m3, self.t_k, self.gamma_oil, self.gamma_gas
    ...
    def _calc_b_oil_in_pb_m3m3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_fvf_Standing_m3m3_saturated(
                self.rsb_m3m3, self.gamma_gas, self.gamma_oil, self.t_k
            )

    def _calc_b_oil_above_pb_m3m3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_fvf_VB_m3m3_above(
                self.b_oil_b_m3m3, self.compr_oil_1mpa, self.pb_mpa, self.p_mpa
            )
        if number_cor == 1:
            # if self.rho_oil_kgm3 == 0 or self.rho_oil_kgm3 is None:
            #    raise TypeError
            self.rho_oil_kgm3 = PVT.unf_density_oil_Mccain(
                self.p_mpa,
                self.pb_mpa,
                self.compr_oil_1mpa,
                self.rs_m3m3,
            ...
            return PVT.unf_fvf_Mccain_m3m3_below(
                self.rho_oil_stkgm3, self.rs_m3m3, self.rho_oil_kgm3, self.gamma_gas
            )

    def _calc_b_oil_below_pb_m3m3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_fvf_Standing_m3m3_saturated(
                self.rs_m3m3, self.gamma_gas, self.gamma_oil, self.t_k
            )
        if number_cor == 1:
            return PVT.unf_fvf_Glaso_m3m3_below(
                self.rs_m3m3, self.t_k, self.gamma_gas, self.gamma_oil, self.p_mpa
            )
        if number_cor == 2:
            return PVT.unf_fvf_Mccain_m3m3_below(
                self.rho_oil_stkgm3, self.rs_m3m3, self.rho_oil_kgm3, self.gamma_gas
    ...
    def _calc_rho_oil_kgm3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_density_oil_Standing(
                self.p_mpa,
                self.pb_mpa,
                self.compr_oil_1mpa,
                self.rs_m3m3,
        ...
        if number_cor == 1:
            return PVT.unf_density_oil_Mccain(
                self.p_mpa,
                self.pb_mpa,
                self.compr_oil_1mpa,
                self.rs_m3m3,
    ...
    def _calc_mu_dead_oil_cp(self, number_cor):
        ...
        if number_cor == 2:
            return PVT.unf_deadoilviscosity_BeggsRobinson_VBA_cP(
                self.gamma_oil, self.t_k
            )

    def _calc_mu_oil_in_pb_cp(self, number_cor):
        if number_cor == 0:
            return PVT.unf_saturatedoilviscosity_Beggs_cP(
                self.mu_dead_oil_cp, self.rsb_m3m3
            )
        if number_cor == 1:
            return PVT.unf_saturatedoilviscosity_Beggs_cP_vba(
                self.mu_dead_oil_cp, self.rsb_m3m3
            )

    def _calc_mu_oil_any_p_cp(self, number_cor):
        if number_cor == 0:
            return PVT.unf_oil_viscosity_Beggs_VB_cP(
                self.mu_dead_oil_cp, self.rs_m3m3, self.p_mpa, self.pb_mpa
            )
        if number_cor == 1:
            return PVT.unf_viscosity_oil_Standing_cP(
                self.rs_m3m3, self.mu_dead_oil_cp, self.p_mpa, self.pb_mpa
            )
    ...
    def _calc_thermal_conduct_oil_wmk(self, number_cor):
        ...
        if number_cor == 1:
            return PVT.unf_thermal_conductivity_oil_Abdul_Seoud_Moharam_WmK(
    ...
    def _calc_pseudocritical_temperature_k(self, number_cor):
        if number_cor == 0:
            return PVT.unf_pseudocritical_temperature_K(
    ...
    def _calc_pseudocritical_pressure_mpa(self, number_cor):
        if number_cor == 0:
            return PVT.unf_pseudocritical_pressure_MPa(
                self.gamma_gas, self.y_h2s, self.y_co2, self.y_n2
            )
        if number_cor == 1:
    ...
    def _calc_zfactor(self, number_cor):
        ...
        if number_cor == 1:
            return PVT.unf_z_factor_Kareem(
                self.t_k / self.tpc_k, self.p_mpa / self.ppc_mpa
            )

    def _calc_mu_gas_cp(self, number_cor):
        if number_cor == 0:
            return PVT.unf_gasviscosity_Lee_cP(
                self.t_k, self.p_mpa, self.z, self.gamma_gas
            )
    ...
    def _calc_compr_gas_1bar(self, number_cor):
        if number_cor == 0:
            return uc.compr_1mpa_2_1bar(
                PVT.unf_compressibility_gas_Mattar_1MPa(
                    self.p_mpa, self.t_k, self.ppc_mpa, self.tpc_k
                )
            )

    def _calc_rho_gas_kgm3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_gas_density_kgm3(
                self.t_k, self.p_mpa, self.gamma_gas, self.z
            )
        if number_cor == 1:
    ...
    def _calc_heatcap_gas_jkgc(self, number_cor):
        if number_cor == 0:
            return PVT.unf_heat_capacity_gas_Mahmood_Moshfeghian_JkgC(
    ...
    def _calc_rho_wat_kgm3(self, number_cor):
        if number_cor == 0:
            return PVT.unf_density_brine_Spivey_kgm3(
                self.t_k, self.p_mpa, self.s_ppm, self.par_wat
            )
        if number_cor == 1:
            return PVT.unf_density_brine_uniflocvba_kgm3(
                self.gamma_wat, self.b_wat_m3m3
            )
        ...
        if number_cor == 0:
            return uc.compr_1mpa_2_1bar(
                PVT.unf_compressibility_brine_Spivey_1MPa(
                    self.t_k, self.p_mpa, self.s_ppm, self.z, self.par_wat
                )
            )
    ...
    def _calc_sigma_oil_gas_nm(self, number_cor):
        if number_cor == 0:
            return PVT.unf_surface_tension_go_Baker_Swerdloff_Nm(
                self.t_k, self.gamma_oil, self.p_mpa
            )
        if number_cor == 1:
            return PVT.unf_surface_tension_go_Abdul_Majeed_Nm(
                self.t_k, self.gamma_oil, self.rs_m3m3
            )
        if number_cor == 2:
            return PVT.unf_surface_tension_Baker_Sverdloff_vba_nm(
                uc.bar2atm(self.p_mpa * 10), self.t_c, self.gamma_oil
            )[0]

    def _calc_sigma_wat_gas_nm(self, number_cor):
        if number_cor == 0:
            return PVT.unf_surface_tension_gw_Sutton_Nm(
    ...
    def calc(self, p_bar, t_c):
        ...
        if self.activate_rus_cor == 0:  # нормальный расчет по UniflocVBA
            ...
            if self.true_rsb_m3t:
                self.rsb_m3t = (
                    self.rsb_m3m3 * 10 ** 3 * 273 / 293 / (self.gamma_oil * 1000)
                )
            else:
                self.rsb_m3t = (
                    self.rsb_m3m3 / self.gamma_oil
                )  # c этой штукой расчет получше
            self.pb_mpa = pvt_rus.calc_pb(
                self.pb_bar_for_rus_cor / 10,
                self.gamma_oil * 1000,
                self.rsb_m3t,
                self.t_res_k,
            ...
            (
                self.gas_liberated_m3t,
                self.gas_dissolved_m3t,
            ) = pvt_rus.unf_calc_gas_liberated_and_dissolved(
                self.t_k,
                self.gamma_oil * 1000,
                self.gamma_oil,
                self.gamma_gas,
            ...
            if self.p_mpa > self.pb_mpa:
                if self.gas_dissolved_m3t_fixed != None and self.rs_m3m3_fixed != None:
                    self.gas_liberated_m3t, self.gas_dissolved_m3t = (
                        0,
                        self.gas_dissolved_m3t_fixed,
                    )  # 73.23
                    self.gas_liberated_m3m3, self.rs_m3m3 = (
                        0,
                        self.rs_m3m3_fixed,
                    )  # 63.01
