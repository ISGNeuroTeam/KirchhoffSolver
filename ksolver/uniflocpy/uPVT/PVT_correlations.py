...
def unf_pb_Valko_MPaa(rsb_m3m3, gamma_oil=0.86, gamma_gas=0.6, t_K=350):
    ...
    z1 = (
        -4.81413889469569
        + 0.748104504934282 * np.log(rsb_m3m3)
        + 0.174372295950536 * np.log(rsb_m3m3) ** 2
        - 0.0206 * np.log(rsb_m3m3) ** 3
    )
...
def unf_rs_Standing_m3m3(
    p_MPaa, pb_MPaa=0, rsb_m3m3=0, gamma_oil=0.86, gamma_gas=0.6, t_K=350
):
    """
...
def unf_rs_Velarde_m3m3(
    p_MPaa, pb_MPaa=10, rsb_m3m3=100, gamma_oil=0.86, gamma_gas=0.6, t_K=350
):
    """
...
def unf_gamma_gas_Mccain(
    rsp_m3m3, rst_m3m3, gamma_gassp, gamma_oil, psp_MPaa=0, tsp_K=0
):
    """
    ...
    if psp_MPaa > 0 and rsp_m3m3 > 0 and rst_m3m3 > 0 and gamma_gassp > 0:
        ...
        z1 = (
            -17.275
            + 7.9597 * np.log(psp_psia)
            - 1.1013 * np.log(psp_psia) ** 2
            + 2.7735 * 10 ** (-2) * np.log(psp_psia) ** 3
            + 3.2287 * 10 ** (-3) * np.log(psp_psia) ** 4
        )
        z2 = (
            -0.3354
            - 0.3346 * np.log(rsp_scfstb)
            + 0.1956 * np.log(rsp_scfstb) ** 2
            - 3.4374 * 10 ** (-2) * np.log(rsp_scfstb) ** 3
            + 2.08 * 10 ** (-3) * np.log(rsp_scfstb) ** 4
        )
        z3 = (
            3.705
            - 0.4273 * api
        ...
        z4 = (
            -155.52
            + 629.61 * gamma_gassp
            - 957.38 * gamma_gassp ** 2
            + 647.57 * gamma_gassp ** 3
            - 163.26 * gamma_gassp ** 4
        )
        z5 = (
            2.085
            - 7.097 * 10 ** (-2) * tsp_F
        ...
        gamma_gas = (gamma_gassp * rsp_scfstb + gamma_gasst * rst_scfstb) / (
...
def unf_fvf_Mccain_m3m3_below(
    density_oilsto_kgm3, rs_m3m3, density_oil_kgm3, gamma_gas
):
    """
...
def unf_compressibility_saturated_oil_McCain_1Mpa(
    p_mpa, pb_mpa, t_k, gamma_oil, rsb_m3m3
):
    """
    ...
    co_1psi = np.exp(
        -7.573
        - 1.450 * np.log(p_psia)
        - 0.383 * np.log(pb_psia)
        + 1.402 * np.log(t_F)
        + 0.256 * np.log(api)
        + 0.449 * np.log(rsb_scfstb)
    )
...
def unf_compressibility_oil_VB_1Mpa(
    rs_m3m3, t_K, gamma_oil, p_MPaa, gamma_gas=0.6
):  # TODO above bubble point!!
    """
    ...
    if p_MPaa > 0:
        ...
        co_1MPa = uc.compr_1psi_2_1pa(
            10 ** 6
            * (-1433 + 5 * rs_scfstb + 17.2 * t_F - 1180 * gamma_gas + 12.61 * api)
            / (10 ** 5 * p_psia)
        )
    else:
        co_1MPa = 0
...
def unf_compessibility_oil_VB_1MPa_vba(rsb_m3m3, t_k, gamma_oil, gamma_gas):
    ...
    return (
        1 / uc.atm2bar(1 / co_1atm) / 10
    )  # *))) **когда не дружишь с математикой ***когда слишком много времени потратил на uniflocpy
...
def unf_fvf_Standing_m3m3_saturated(rs_m3m3, gamma_gas, gamma_oil, t_K):
    ...
    bo = (
        ...
        + 1.47
        * 10 ** (-4)
        * (rs_scfstb * (gamma_gas / gamma_oil) ** 0.5 + 1.25 * t_F) ** 1.175
    )
    return bo


def unf_density_oil_Mccain(
    p_MPaa, pb_MPaa, co_1MPa, rs_m3m3, gamma_gas, t_K, gamma_oil, gamma_gassp=0
):
    """
    ...
    while abs(ro_po_current - ro_po_previous) > epsilon and counter < maxiter:
        ...
        ro_a = (
            -49.8930
            + 85.0149 * gamma_gassp
            - 3.70373 * gamma_gassp * ro_po
            + 0.0479818 * gamma_gassp * ro_po ** 2
            + 2.98914 * ro_po
            - 0.0356888 * ro_po ** 2
        )
        ro_po = (rs_scfstb * gamma_gas + 4600 * gamma_oil) / (
            73.71 + rs_scfstb * gamma_gas / ro_a
        )
        ro_po_current = ro_po
        counter = counter + 1
    p_psia = uc.Pa2psi(p_MPaa * 10 ** 6)
    ...
    if p_MPaa < pb_MPaa:
        dro_p = (0.167 + 16.181 * 10 ** (-0.0425 * ro_po)) * (p_psia / 1000) - 0.01 * (
            0.299 + 263 * 10 ** (-0.0603 * ro_po)
        ) * (p_psia / 1000) ** 2
        ro_bs = ro_po + dro_p
        dro_t = (0.00302 + 1.505 * ro_bs ** (-0.951)) * (t_F - 60) ** 0.938 - (
            0.0216 - 0.0233 * 10 ** (-0.0161 * ro_bs)
        ) * (t_F - 60) ** 0.475
        ro_or = ro_bs - dro_t
    else:
...
def unf_density_oil_Standing(
    p_MPaa, pb_MPaa, co_1MPa, rs_m3m3, bo_m3m3, gamma_gas, gamma_oil
):
    """
...
def unf_deadoilviscosity_Standing(gamma_oil, t_k):
    return (0.32 + 1.8 * 10 ** 7 / (141.5 / gamma_oil - 131.5) ** 4.53) * (
        360 / (1.8 * t_k - 260)
    ) ** (10 ** (0.43 + 8.33 / (141.5 / gamma_oil - 131.5)))
...
def unf_viscosity_oil_Standing_cP(rs_m3m3, Dead_oil_viscosity, p_MPa, pb_MPa):
    ...
    b = (
        0.68 / 10 ** (0.000484 * rs_m3m3)
        + 0.25 / 10 ** (0.006176 * rs_m3m3)
        + 0.062 / 10 ** (0.021 * rs_m3m3)
    )
    ...
    if pb_MPa < p_MPa:
        unf_pvt_viscosity_oil_Standing_cP = (
            ...
            * (p_MPa - pb_MPa)
            * (
                0.024 * unf_pvt_viscosity_oil_Standing_cP ** 1.6
                + 0.038 * unf_pvt_viscosity_oil_Standing_cP ** 0.56
            )
        )
...
def unf_undersaturatedoilviscosity_Petrovsky_cP(
    p_MPaa, pb_MPaa, bubblepointviscosity_cP
):
    """
        Viscosity correlation for pressure above bubble point
    ...
    A = (
        -1.0146
        + 1.3322 * np.log(bubblepointviscosity_cP)
        - 0.4876 * np.log(bubblepointviscosity_cP) ** 2
        - 1.15036 * np.log(bubblepointviscosity_cP) ** 3
    )
    p_psia = uc.Pa2psi(p_MPaa * 10 ** 6)
    pb_psia = uc.Pa2psi(pb_MPaa * 10 ** 6)
    viscosity_cP = (
        bubblepointviscosity_cP + 1.3449 * 10 ** (-3) * (p_psia - pb_psia) * 10 ** A
    )
    return viscosity_cP
...
def unf_oil_viscosity_Beggs_VB_cP(deadoilviscosity_cP, rs_m3m3, p_MPaa, pb_MPaa):
    ...
    :return: oil viscosity,cP
    """
    saturatedviscosity_cP = unf_saturatedoilviscosity_Beggs_cP(
        deadoilviscosity_cP, rs_m3m3
    )
    if p_MPaa <= pb_MPaa:
        viscosity_cP = saturatedviscosity_cP
    else:
        viscosity_cP = unf_undersaturatedoilviscosity_VB_cP(
            p_MPaa, pb_MPaa, saturatedviscosity_cP
        )
    return viscosity_cP
...
def unf_pb_Glaso_MPaa(rs_m3m3, t_K, gamma_oil, gamma_gas):
    ...
    pb = (
        uc.psi2Pa(10 ** (1.7669 + 1.7447 * np.log10(pb) - 0.30218 * np.log10(pb) ** 2))
        / 10 ** 6
    )
    return pb
...
def unf_fvf_Glaso_m3m3_below(rs_m3m3, t_K, gamma_oil, gamma_gas, p_MPaa):
    ...
    bt = (
        ...
        * t_F ** 0.5
        / gamma_gas ** 0.3
        * gamma_oil ** (2.9 * 10 ** (-0.00027 * rs_scfstb))
        * p_psia ** (-1.1089)
    )
    bt = 10 ** (
        8.0135 * 10 ** (-2)
        + 4.7257 * 10 ** (-1) * np.log10(bt)
        + 1.7351 * 10 ** (-1) * np.log10(bt) ** 2
    )
    return bt
...
def unf_McCain_specificgravity(p_MPaa, rsb_m3m3, t_K, gamma_oil, gamma_gassp):
    ...
    gamma_gasr = 1 / (
        ...
        + 3.38346 / t_F ** 0.5
        - 0.000992 * t_F
        - 0.000081147 * rsb_scfstb
        - 0.001956 * api
        + 1.081956 / gamma_gassp
        + 0.394035 * gamma_gassp ** 2
    )
    return gamma_gasr
...
def unf_heat_capacity_oil_Wes_Wright_JkgC(gamma_oil, t_c):
    ...
    """
    return (
        (2 * 10 ** (-3) * t_c - 1.429) * gamma_oil + (2.67 * 10 ** (-3)) * t_c + 3.049
    ) * 1000
...
def unf_pseudocritical_temperature_K(gamma_gas, y_h2s=0.0, y_co2=0.0, y_n2=0.0):
    ...
    J = (
        1.1582 * 10 ** (-1)
        - 4.5820 * 10 ** (-1) * y_h2s * (tc_h2s_R / pc_h2s_psia)
        - 9.0348 * 10 ** (-1) * y_co2 * (tc_co2_R / pc_co2_psia)
        - 6.6026 * 10 ** (-1) * y_n2 * (tc_n2_R / pc_n2_psia)
        + 7.0729 * 10 ** (-1) * gamma_gas
        - 9.9397 * 10 ** (-2) * gamma_gas ** 2
    )
    K = (
        3.8216
        - 6.5340 * 10 ** (-2) * y_h2s * (tc_h2s_R / pc_h2s_psia)
        - 4.2113 * 10 ** (-1) * y_co2 * (tc_co2_R / pc_co2_psia)
        - 9.1249 * 10 ** (-1) * y_n2 * (tc_n2_R / pc_n2_psia)
        + 1.7438 * 10 * gamma_gas
        - 3.2191 * gamma_gas ** 2
    )
    tpc_R = K ** 2 / J
...
def unf_compressibility_gas_Mattar_1MPa(p_MPaa, t_K, ppc_MPa, tpc_K):
    ...
    z_derivative = (
        ...
        * solution[1]
        / tpr ** 3
        * (1 + 0.7210 * solution[1] ** 2 - 0.7210 ** 2 * solution[1] ** 4)
        * np.exp(-0.7210 * solution[1] ** 2)
    )
    cpr = 1 / solution[1] - 0.27 / (solution[0] ** 2 * tpr) * (
        z_derivative / (1 + solution[1] * z_derivative / solution[0])
    )
    cg = cpr / ppc_MPa
    return cg
...
def unf_gas_fvf_m3m3(t_K, p_MPaa, z):
    ...
    bg = (
        101.33 * 10 ** (-3) * t_K * z / (1 * 293.15 * p_MPaa)
    )  # тут от нормальный условий по температуре
    return bg
...
def unf_heat_capacity_gas_Mahmood_Moshfeghian_JkgC(p_MPaa, t_K, gamma_gas):
    ...
    f = 0.0214
    return (
        (a * (b ** t_c) * (t_c ** c) + d * (e ** p_MPaa) * (p_MPaa ** f))
        * ((gamma_gas / 0.60) ** 0.025)
    ) * 1000
...
def unf_density_brine_Spivey_kgm3(t_K, p_MPaa, s_ppm, par=1):
    ...
    ro_w70 = (-0.127213 * (t_C / 100) ** 2 + 0.645486 * (t_C / 100) + 1.03265) / (
        -0.070291 * (t_C / 100) ** 2 + 0.639589 * (t_C / 100) + 1
    )
    ...
    fw = (-11.403 * (t_C / 100) ** 2 + 29.932 * (t_C / 100) + 27.952) / (
        0.20684 * (t_C / 100) ** 2 + 0.3768 * (t_C / 100) + 1
    )
    ...
    d_m1 = (
        -1.1149 * 10 ** (-4) * (t_C / 100) ** 2
        + 1.7105 * 10 ** (-4) * (t_C / 100)
        - 4.3766 * 10 ** (-4)
    )
    d_m2 = (
        -8.878 * 10 ** (-4) * (t_C / 100) ** 2
        - 1.388 * 10 ** (-4)
        - 2.96318 * 10 ** (-3)
    ) / (0.51103 * (t_C / 100) + 1)
    d_m3 = (
        2.1466 * 10 ** (-3) * (t_C / 100) ** 2
        + 1.2427 * 10 ** (-2) * (t_C / 100)
        + 4.2648 * 10 ** (-2)
    ) / (-8.1009 * 10 ** (-2) * (t_C / 100) ** 2 + 0.525417 * (t_C / 100) + 1)
    d_m4 = (
        2.356 * 10 ** (-4) * (t_C / 100) ** 2
        - 3.636 * 10 ** (-4) * (t_C / 100)
        - 2.278 * 10 ** (-4)
    )
    ...
    eb = ew + 0.1249
    f_m1 = (-0.617 * (t_C / 100) ** 2 - 0.747 * (t_C / 100) - 0.4339) / (
        10.26 * (t_C / 100) + 1
    )
    ...
    if s == 0:
        ...
        c = (0.6855 * (t_C / 100) ** 2 - 3.1992 * (t_C / 100) - 3.7968) / (
            0.07711 * (t_C / 100) ** 2 + 0.2229 * (t_C / 100) + 1
        )
        # Растворимость метана в чистой воде
        m_ch4_w = np.exp(
            a * (np.log(p_MPaa - p_sigma)) ** 2 + b * np.log(p_MPaa - p_sigma) + c
        )
        ...
        ro_b_ch4 = (1000 + m * 58.4428 + m_ch4_b * 16.043) / (
            (1000 + m * 58.4428) * v_b0 + m_ch4_b * v_ch4_b
        )
        ro = ro_b_ch4
    else:
        ro = 0
...
def unf_compressibility_brine_Spivey_1MPa(t_K, p_MPaa, s_ppm, z=1.0, par=1):
    ...
    c_b_ch4_u = -(
        (1000 + m * 58.4428) * dderivative1 + m_ch4_b * derivative_v_ch4_b
    ) / ((1000 + m * 58.4428) * v_b0 + m_ch4_b * v_ch4_b)
    # Найдем сжимаемость раствора в случае двуфазной системы
    # Необходимая производная от растворимости метана по давлению
    derivative_m_ch4_b = m_ch4_w * (
        (2 * a * np.log(p_MPaa - p_sigma) + b) / (p_MPaa - p_sigma)
        - 2 * m * derivative2
    )
    ...
    c_b_ch4_s = -(
        (1000 + m * 58.4428) * dderivative1
        + m_ch4_b * derivative_v_ch4_b
        + derivative_m_ch4_b * (v_ch4_b - vm_ch4_g)
    ) / ((1000 + m * 58.4428) * v_b0 + m_ch4_b * v_ch4_b)
    if s == 0:
        c_1MPa = c_w
    elif par == 0:
...
def unf_fvf_brine_McCain_m3m3(t_K, p_MPaa):
    ...
    dvwp = (
        -1.95301 * 10 ** (-9) * p_psi * t_f
        - 1.72834 * 10 ** (-13) * p_psi ** 2 * t_f
        - 3.58922 * 10 ** (-7) * p_psi
        - 2.25341 * 10 ** (-10) * p_psi ** 2
    )
    dvwt = (
        -1.0001 * 10 ** (-2)
        + 1.33391 * 10 ** (-4) * t_f
...
def unf_fvf_brine_Spivey_m3m3(t_K, p_MPaa, s_ppm):  # TODO check
    ...
    ro_w70_sc = (-0.127213 * (20 / 100) ** 2 + 0.645486 * (20 / 100) + 1.03265) / (
        -0.070291 * (20 / 100) ** 2 + 0.639589 * (20 / 100) + 1
    )
    # Температурные коэффициенты сжимаемости чистой воды
    ew = (4.221 * (t_C / 100) ** 2 - 3.478 * (t_C / 100) + 6.221) / (
        0.5182 * (t_C / 100) ** 2 - 0.4405 * (t_C / 100) + 1
    ...
    fw_sc = (-11.403 * (20 / 100) ** 2 + 29.932 * (20 / 100) + 27.952) / (
        0.20684 * (20 / 100) ** 2 + 0.3768 * (20 / 100) + 1
    )
    # Температурные коэффициенты плотности раствора
    d_m1 = (
        -1.1149 * 10 ** (-4) * (t_C / 100) ** 2
    ...
    f_m2 = (9.917 * (t_C / 100) + 5.1128) / (3.892 * (t_C / 100) + 1)
    f_m3 = 0.0365 * (t_C / 100) ** 2 - 0.0369 * (t_C / 100)
    f_m1_sc = (-0.617 * (20 / 100) ** 2 - 0.747 * (20 / 100) - 0.4339) / (
        10.26 * (20 / 100) + 1
    )
    ...
    v_b0 = 1 / ro_b
    v_b0_sc = 1 / ro_b_sc
    bw = ((1000 + m * 58.4428) * v_b0 + m_ch4_b * v_ch4_b) / (
        (1000 + m * 58.4428) * v_b0_sc
    )
    return bw
...
def unf_gwr_brine_Spivey_m3m3(s_ppm, z):  # TODO check
    ...
    # Первым шагом вычисляется плотность чистой воды при давлении 70 MPa и температуре
    ro_w70_sc = (
        -0.127213 * (t_C_sc / 100) ** 2 + 0.645486 * (t_C_sc / 100) + 1.03265
    ) / (-0.070291 * (t_C_sc / 100) ** 2 + 0.639589 * (t_C_sc / 100) + 1)
    # Температурные коэффициенты сжимаемости чистой воды
    ew_sc = (4.221 * (t_C_sc / 100) ** 2 - 3.478 * (t_C_sc / 100) + 6.221) / (
        0.5182 * (t_C_sc / 100) ** 2 - 0.4405 * (t_C_sc / 100) + 1
    )
    fw_sc = (-11.403 * (t_C_sc / 100) ** 2 + 29.932 * (t_C_sc / 100) + 27.952) / (
        0.20684 * (t_C_sc / 100) ** 2 + 0.3768 * (t_C_sc / 100) + 1
    )
    # Температурные коэффициенты плотности раствора
    d_m1 = (
        -1.1149 * 10 ** (-4) * (t_C_sc / 100) ** 2
        + 1.7105 * 10 ** (-4) * (t_C_sc / 100)
        - 4.3766 * 10 ** (-4)
    )
    d_m2 = (
        -8.878 * 10 ** (-4) * (t_C_sc / 100) ** 2
        - 1.388 * 10 ** (-4)
        - 2.96318 * 10 ** (-3)
    ) / (0.51103 * (t_C_sc / 100) + 1)
    d_m3 = (
        2.1466 * 10 ** (-3) * (t_C_sc / 100) ** 2
        + 1.2427 * 10 ** (-2) * (t_C_sc / 100)
        + 4.2648 * 10 ** (-2)
    ) / (-8.1009 * 10 ** (-2) * (t_C_sc / 100) ** 2 + 0.525417 * (t_C_sc / 100) + 1)
    d_m4 = (
        2.356 * 10 ** (-4) * (t_C_sc / 100) ** 2
        - 3.636 * 10 ** (-4) * (t_C_sc / 100)
        - 2.278 * 10 ** (-4)
    )
    # Плотность раствора воды с хлоридом натрия при давлении 70 MPa и температуре
    ...
    # Температурные коэффициенты сжимаемости раствора
    eb_sc = ew_sc + 0.1249
    f_m1 = (-0.617 * (t_C_sc / 100) ** 2 - 0.747 * (t_C_sc / 100) - 0.4339) / (
        10.26 * (t_C_sc / 100) + 1
    )
    ...
    c = (0.6855 * (t_C_sc / 100) ** 2 - 3.1992 * (t_C_sc / 100) - 3.7968) / (
        0.07711 * (t_C_sc / 100) ** 2 + 0.2229 * (t_C_sc / 100) + 1
    )
    # Растворимость метана в чистой воде
    m_ch4_w = np.exp(
        a * (np.log(p_MPaa_sc - p_sigma)) ** 2 + b * np.log(p_MPaa_sc - p_sigma) + c
    )
    # Далее найдем коэффициенты взаимодействия
    lyambda = (
        -0.80898
        + 1.0827 * 10 ** (-3) * t_C_sc
        + 183.85 / t_C_sc
        + 3.924 * 10 ** (-4) * p_MPaa_sc
        - 1.97 * 10 ** (-6) * p_MPaa_sc ** 2
    )
    dzeta = -3.89 * 10 ** (-3)
    # Растворимость метана в растворе
    # нужно отметить, что в обозначениях было сложно разобраться, для понимания этой формулы лучше читать статью
...
def unf_viscosity_brine_McCain_cp(t_K, p_MPaa, s_ppm):  # TODO check
    ...
    b = (
        ...
        + 0.0263951 * wpTDS
        - 0.000679461 * wpTDS ** 2
        - 5.47119 * 10 ** (-5) * wpTDS ** 3
        + 1.55586 * 10 ** (-6) * wpTDS ** 4
    )

    visc = a * (1.8 * t_K - 460) ** b
    p_psi = uc.bar2psi(p_MPaa * 10)
    viscosity_cp = visc * (
        0.9994 + 4.0295 * 10 ** (-5) * p_psi + 3.1062 * 10 ** (-9) * p_psi ** 2
    )
    return viscosity_cp
...
def unf_viscosity_brine_MaoDuan_cP(t_K, p_MPaa, s_ppm):  # TODO check
    ...
    viscosity = np.exp(
        0.28853170 * 10 ** 7 * t_K ** (-2)
        - 0.11072577 * 10 ** 5 * t_K ** (-1)
        - 0.90834095 * 10
        + 0.30925651 * 10 ** (-1) * t_K
        - 0.27407100 * 10 ** (-4) * t_K ** 2
        + ro_w
        * (
            -0.19298951 * 10 ** 7 * t_K ** (-2)
            + 0.56216046 * 10 ** 4 * t_K ** (-1)
            + 0.13827250 * 10 ** 2
            - 0.47609523 * 10 ** (-1) * t_K
            + 0.35545041 * 10 ** (-4) * t_K ** 2
        )
    )
    ...
    a = (
        -0.213119213
        + 0.13651589 * 10 ** (-2) * t_K
        - 0.12191756 * 10 ** (-5) * t_K ** 2
    )
    b = (
        -0.69161945 * 10 ** (-1)
        - 0.27292263 * 10 ** (-3) * t_K
        + 0.20852448 * 10 ** (-6) * t_K ** 2
    )
...
def unf_heat_capacity_water_IAPWS_JkgC(t_c):  # TODO заменить
    ...
    if t_c < 95:
        ...
        return (
            cor_in_range_5_95(95)
            + (cor_in_range_5_95(95) - cor_in_range_5_95(85)) / 10 * (t_c - 95)
        ) * 1000
...
def unf_thermal_conductivity_water_IAPWS_WmC(t_c):  # TODO заменить
    ...
    def cor_in_range_5_95(t_c):
        return 0.5636 + 1.946 * 10 ** (-3) * t_c - 8.151 * 10 ** (-6) * t_c ** 2

    if t_c < 95:
        return cor_in_range_5_95(t_c)
    else:
        return cor_in_range_5_95(95) + (
            cor_in_range_5_95(95) - cor_in_range_5_95(85)
        ) / 10 * (t_c - 95)
...
def unf_surface_tension_go_Abdul_Majeed_Nm(t_K, gamma_oil, rs_m3m3):
    ...
    """
    t_C = uc.k2c(t_K)
    surface_tension_dead_oil_dynes_cm = (
        1.17013 - 1.694 * 10 ** (-3) * (1.8 * t_C + 32)
    ) * (38.085 - 0.259 * (141.5 / gamma_oil - 131.5))
    relative_surface_tension_go_od = 0.056379 + 0.94362 * np.exp(
        -21.6128 * 10 ** (-3) * rs_m3m3
    )
    surface_tension_dynes_cm = (
        surface_tension_dead_oil_dynes_cm * relative_surface_tension_go_od
    )
...
def unf_surface_tension_go_Baker_Swerdloff_Nm(t_K, gamma_oil, p_MPa):
    ...
    if t_C <= 20:
        ...
        surface_tension_dead_oil_dynes_cm = (
            surface_tension_dead_oil_20_c_dynes_cm
            - (t_C - 20)
            * (
                surface_tension_dead_oil_20_c_dynes_cm
                - surface_tension_dead_oil_38_c_dynes_cm
            )
            / 18
        )
    surface_tension_go_Baker_Swerdloff_dynes_cm = (
        surface_tension_dead_oil_dynes_cm * np.exp(-125.1763 * 10 ** (-4) * p_bar)
    )
...
@lru_cache(maxsize=None)
def unf_surface_tension_gw_Sutton_Nm(
    rho_water_kgm3, rho_gas_kgm3, t_c
):  # TODO поправка на соленость добавить
    """
    ...
    t_r = uc.c2r(t_c)
    surface_tension_dyncm = (
        (1.53988 * (rho_water_gcm3 - rho_gas_gcm3) + 2.08339)
        / ((t_r / 302.881) ** (0.821976 - 0.00183785 * t_r + 0.00000134016 * t_r ** 2))
    ) ** 3.6667
...
def coef_mt(t_k, rho_deadoil_kgm3, gamma_gas):
    return 1 + 0.029 * (t_k - 293) * (
        rho_deadoil_kgm3 * gamma_gas * 10 ** (-3) - 0.7966
    )


def coef_dt(t_k, rho_deadoil_kgm3, gamma_gas):
    return (
        10 ** (-3) * rho_deadoil_kgm3 * gamma_gas * (4.5 - 0.00305 * (t_k - 293))
        - 4.785
    )
...
def unf_calc_gas_liberated_and_dissolved(
    t_k,
    rho_deadoil_kgm3,
    gamma_oil,
    gamma_gas,
    ...
    if return_m3m3:
        return (
            gas_liberated_m3t * rho_deadoil_kgm3 / 1000,
            gas_dissolved_m3t * rho_deadoil_kgm3 / 1000,
        )
...
def calc_gas_for_fsolve(
    ...
    gas_dissolved_m3m3 = unf_calc_gas_liberated_and_dissolved(
        t_k,
        rho_deadoil_kgm3,
        gamma_oil,
        gamma_gas,
...
def calc_gas_with_fsolve(
    ...
    answer = answer.x[0]
    return unf_calc_gas_liberated_and_dissolved(
        t_k, rho_deadoil_kgm3, gamma_oil, gamma_gas, p_mpa, pb_mpa, answer, return_m3m3
    )
