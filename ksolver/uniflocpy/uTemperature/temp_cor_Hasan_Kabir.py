...
class Hasan_Kabir_cor:
    ...
    def calc_annulus(self, delta_t_an_c):
        ...
        self.number_pr_an = (
            uc.cP2pasec(self.mu_an_cP)
            * self.heatcap_an_jkgc
            / self.thermal_conduct_an_wmk
        )
        self.number_gr_an = (
            (self.r_cas_in_m - self.r_tube_out_m) ** 3
            * self.rho_an_kgm3 ** 2
            * self.heat_expansion_an_1c
            * uc.g
            * self.delta_t_an_c
            / uc.cP2pasec(self.mu_an_cP) ** 2
        )

        self.han_wm2c = (
            0.049
            * (self.number_gr_an * self.number_pr_an) ** (1 / 3)
            * self.number_pr_an ** 0.074
            * self.thermal_conduct_an_wmk
        ...
        self.overall_heat_transfer_coef_wm2c = (
            1
            / self.r_tube_out_m
            * (
                1
        ...
        self.delta_t_an_c = self.heat_transfer_rate / (
            2 * uc.pi * self.r_tube_in_m * self.han_wm2c
        )
    ...
    def calc_grad_t_cm(self, p_bar, t_c):
        ...
        self.heat_dissipation_to_earth = math.log(
            math.exp(-0.2 * self.time_dimensionless)
        ...
        self.thermal_resistance_cond_cas = (
            math.log(self.r_cas_out_m / self.r_cas_in_m) / self.thermal_conduct_cas_wmk
        )
        self.thermal_resistance_cond_cem = (
            math.log(self.r_well_m / self.r_cas_out_m) / self.thermal_conduct_cem_wmc
        )

        self.thermal_resistance_cond_earth = (
            self.heat_dissipation_to_earth / self.thermal_conduct_earth_wmc
        )
            ...
            * (
                ...
                * self.thermal_conduct_earth_wmc
                / (
                    ...
                    + self.r_tube_out_m
                    * self.overall_heat_transfer_coef_wm2c
                    * self.heat_dissipation_to_earth
                )
            )
        )
        ...
        # self.part_grad_flow = - 1 / self.heatcapn_jkgc * self.heat_flowrate / self.mass_flowraten_kgsec
        self.part_grad_potential = (
            1 / self.heatcapn_jkgc * uc.g * math.sin(self.angle_rad)
        )
        self.part_grad_kinetic = (
            1 / self.heatcapn_jkgc * self.vm_msec * self.grad_v_msecm
        )
