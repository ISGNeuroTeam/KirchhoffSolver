...
class Friction:
    ...
    def __init__(self):
        ...
        self.u_s = (
            1  # подстроечный параметр для сбивки к экспериментальным исследованиям
        )
    ...
    def __correlation__(self, f):
        ...
        """
        self.f = f
        in_log10_part_first = (1.547 / self.number_re / self.f ** (1 / 2)) ** (
            0.9445 * self.u_s
        )
        in_log10_part_second = (self.absolute_roughness_m / 3.7 / self.d_m) ** self.u_s
        result = (
            1 / (-2 / self.u_s * np.log10(in_log10_part_first + in_log10_part_second))
        ) ** 2
    ...
    def calc_f(self, number_re, epsilon_m, d_m):
        ...
        if 2300 < self.number_re <= 3100:  # Turbulent
            p1 = 64 / 2300
            p2 = 0.04
            self.f = p1 + (p2 - p1) / (3100 - 2300) * (
                self.number_re - 2300
            )  # TODO должна быть прямая линия
        if 3100 < self.number_re <= 20000:
            mistake = sp.root(
                self.__correlation__, 0.02, method=method
            ).x  # TODO разобраться с выбором начального приближения
            p3 = float(self.f)
            p2 = 0.04
            self.f = p2 + (p3 - p2) / (20000 - 3100) * (
                self.number_re - 3100
            )  # TODO должна быть прямая линия
