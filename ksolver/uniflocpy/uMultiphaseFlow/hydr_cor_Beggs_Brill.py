...
import math
import ksolver.uniflocpy.uMultiphaseFlow.friction_Bratland as fr  # модуль для расчета коэффициента трения
        ...
        if number_re == 0:
            ...
            if number_re > 2000:
                f_n = (
                    2
                    * np.log10(
                        2 / 3.7 * relative_roughness
                        - 5.02
                        / number_re
                        * np.log10(2 / 3.7 * relative_roughness + 13 / number_re)
                    )
                ) ** -2
                result = 20
                i = 0
                while result > 0.001 or i < 19:
                    new_fn = (
                        ...
                        - 2
                        * np.log10(
                            2 * relative_roughness + 18.7 / (number_re * f_n ** 0.5)
                        )
                    ) ** -2
...
class Beggs_Brill_cor:
    ...
    def __init__(
        self,
        epsilon_friction_m=18.288 * 10 ** (-6),
        angle_grad=90,
        friction_type=1,
        pains_corr_using=0,
        gravity_grad_coef=1,
        friction_grad_coef=1,
        acceleration_grad_coef=1,
        ...
        self.liquid_content = (
            None  # объемное содержание жидкости при отсутствии проскальзывания (3.8)
        )
    ...
    def __calc_hltetta__(self):
        ...
        if self.flow_regime == 2:
            ...
            b = 0.5824
            c = 0.0609
        self.liquid_content_with_zero_angle = (
            a * self.liquid_content ** b / self.val_number_Fr ** c
        )

        self.Nlv = self.vsl_msec * (
            self.rho_liq_kgm3 / (const_g_m2sec * self.sigma_liq_Nm)
        ) ** (1 / 4)
        ...
        if self.flow_regime == 2:
            ...
            result = (1 - self.liquid_content) * math.log(
                d * self.liquid_content ** e * self.Nlv ** f * self.val_number_Fr ** g
            )
            if result <= 0:
                self.correction_factor_c = 0
            else:
        ...
        self.angle_rad = (
            self.angle_grad * math.pi / 180
        )  # TODO если скважина вертикальная, будет коррекция

        self.angle_correction_factor = 1 + self.correction_factor_c * (
            (math.sin(1.8 * self.angle_rad))
            - (1 / 3) * (math.sin(1.8 * self.angle_rad)) ** 3
        )
        self.liquid_content_with_angle = (
            self.liquid_content_with_zero_angle * self.angle_correction_factor
        )

        if self.angle_grad > 0:  # uphill flow
            if self.pains_corr_using == 1:
                self.liquid_content_with_Pains_cor = (
                    0.924 * self.liquid_content_with_angle
                )
            else:
            ...
            if self.pains_corr_using == 1:
                self.liquid_content_with_Pains_cor = (
                    0.685 * self.liquid_content_with_angle
                )
            else:
                self.liquid_content_with_Pains_cor = 1 * self.liquid_content_with_angle
    ...
    def determine_flow_pattern(self, number_Fr, liquid_content):
        ...
        if (self.liquid_content < 0.01 and number_Fr < self.L1) or (
            self.liquid_content >= 0.01 and number_Fr < self.L2
        ):
            ...
            if self.L2 <= number_Fr < self.L3 and self.liquid_content >= 0.01:
                ...
                if (
                    0.01 <= self.liquid_content < 0.4 and self.L3 < number_Fr <= self.L1
                ) or (self.liquid_content >= 0.4 and self.L3 < number_Fr <= self.L4):
                    # Intermittent Flow - прерывистый режим
                    self.flow_regime = 1
                if (self.liquid_content < 0.4 and number_Fr >= self.L1) or (
                    self.liquid_content >= 0.4 and number_Fr > self.L4
                ):
    ...
    def calc_grad(self, p_bar, t_c):
        ...
        if self.p_pa <= 0:
            ...
            if self.flow_regime != 3:
                ...
                B = 1 - A
                self.liquid_content_with_Pains_cor = (
                    A * hltetta_segr + B * hltetta_inter
                )
                self.flow_regime = 3
            ...
            self.friction_grad_pam = (
                self.result_friction * self.rhon_kgm3 * self.vm_msec ** 2 / 2 / self.d_m
            ) * self.friction_grad_coef

            self.density_grad_pam = (
                self.rhos_kgm3
                * const_g_m2sec
                * math.sin(self.angle_rad)
                * self.gravity_grad_coef
            )

            self.result_grad_pam = (
                (self.friction_grad_pam + self.density_grad_pam)
                / (1 - self.Ek)
                * self.acceleration_grad_coef
            )

            self.acceleration_grad_pam = self.result_grad_pam * self.Ek

            self.friction_grad_part_percent = (
                self.friction_grad_pam / self.result_grad_pam * 100
            )

            self.density_grad_part_percent = (
                self.density_grad_pam / self.result_grad_pam * 100
            )

            self.acceleration_grad_part_percent = (
                self.acceleration_grad_pam / self.result_grad_pam * 100
            )
