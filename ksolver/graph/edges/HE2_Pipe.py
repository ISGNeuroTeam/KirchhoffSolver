...
from ksolver.fluids.HE2_Fluid import HE2_DummyWater, HE2_BlackOil, gimme_dummy_BlackOil
...
class HE2_WaterPipeSegment:
    ...
    def __init__(
        self, fluid=None, inner_diam_m=None, roughness_m=None, L_m=None, uphill_m=None
    ):
    ...
    def set_pipe_geometry(self, dx=None, dy=None, L=None, angle=None):
        if dx is not None and dy is not None:
            ...
            logger.error(
                f"This way to set up pipe geometry is undefined, L={L}, dx={dx}"
            )
            ...
            if dx <= 0:
                logger.error(
                    f"This way to set up pipe geometry is undefined, angle={angle}, dx={dx}"
                )
                raise ValueError("")
    ...
    def calc_segment_pressure_drop_slow(
...
class HE2_WaterPipe(abc.HE2_ABC_GraphEdge):
    def __init__(self, dxs, dys, diams, rghs):
        ...
        self._printstr = ";\n ".join(
            [" ".join([f"{itm:.2f}" for itm in vec]) for vec in [dxs, dys, diams, rghs]]
        )
...
class HE2_OilPipeSegment:
    ...
    def calc_P_friction_gradient_Pam(
        self, P_bar, T_C, X_kgsec, fric_sign, grav_sign, calc_direction
    ):
        ...
        P_fric_grad_Pam, P_grav_grad_Pam = mb.calculate(
            current_mishenko, angle, self.inner_diam_m, self.roughness_m
        )
    ...
    def calc_segment_pressure_drop(
        self, P_bar, T_C, X_kgsec, calc_direction, unifloc_direction=-1
    ):
        check_for_nan(P_bar=P_bar, T_C=T_C, X_kgsec=X_kgsec)
        ...
        P_fric_grad_Pam, P_grav_grad_Pam = self.calc_P_friction_gradient_Pam(
            P_bar, T_C, abs(X_kgsec), fric_sign, grav_sign, calc_direction
        )
        ...
        P_drop_bar = fric_sign * uc.Pa2bar(dP_full_Pa) + grav_sign * uc.Pa2bar(
            dP_full_grav_Pa
        )
...
class HE2_OilPipe(abc.HE2_ABC_GraphEdge):
    def __init__(self, dxs, dys, diams, rghs, fluid=None):
        ...
        self.fluid = fluid
        for dx, dy, diam, rgh in zip(dxs, dys, diams, rghs):
            seg = HE2_OilPipeSegment(
                inner_diam_m=diam, roughness_m=rgh, L_m=None, uphill_m=None, fluid=fluid
            )
    ...
    def perform_calc(self, P_bar, T_C, X_kgsec, unifloc_direction):
        ...
        flow_direction = 1 if unifloc_direction % 10 == 1 else -1
        if calc_direction == 1:
            return self.perform_calc_forward(
                P_bar, T_C, flow_direction * abs(X_kgsec)
            )  #!
        else:
            return self.perform_calc_backward(P_bar, T_C, flow_direction * abs(X_kgsec))
