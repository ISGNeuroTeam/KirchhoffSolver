...
from ksolver.graph.nodes.HE2_Vertices import is_junction
...
def evalute_network_fluids_with_root(solver_G, x_dict, have_to_reduce=False):
    ...
    cocktails = restore_cocktail_from_reduced_graph(
        cocktails2, srcs, x_dict2, **mappings
    )
...
def restore_cocktail_from_reduced_graph(
    cocktails3, srcs, x_dict, fluid_em, x_pos_em, x_neg_em
):
    cocktails = {}
    ...
    sink_cocktails = {k:v for k, v in cocktails3.items() if type(k) != tuple}
    cocktails.update(sink_cocktails)
...
def make_small_graph_for_mixer(
    solver_G: nx.DiGraph, removeRoot: bool = True
    ...
    for u, v, k in new_G.edges:
        fluid_em[(u, v, k)] = [
            (u, v)
        ]  # key - edge in reduced graph, value - list of original graph edges
        x_pos_em[(u, v, k)] = [(u, v)]
        x_neg_em[(u, v, k)] = []
        assert k == 0
...
def evalute_network_fluids_wo_root(_G, x_dict):
    ...
    A = (
        -1
        * nx.incidence_matrix(
            G, nodelist=nodes, edgelist=edges, oriented=True
        ).toarray()
    )
    Q = np.matmul(A, x)
    ...
    for i, node in enumerate(nodes):
        ...
        if (
            node in sinks
        ):  # n is sink, so there is an unknown (equation variable) for outbound flow
        ...
        if (
            len(these_vars_are_the_same) > 1
        ):  # if there are more than one remembered outlet flow
            for fl1, fl2 in zip(
                these_vars_are_the_same[1:], these_vars_are_the_same[:-1]
            ):
                mx2[
                    i2, fl1
                ] = 1  # we have to right an equation to matrix, to describe that these outlet fluids are equal
                mx2[i2, fl2] = -1
                i2 += 1
