...
def evalute_network_fluids_with_root_new_version_with_reduce(G, x_dict):
    ...
    cocktails2, srcs = restore_cocktail_from_reduced_graph(
        cocktails3, srcs, reducing_stack
    )
...
def make_small_graph_for_mixer(
    src_G: nx.DiGraph,
) -> Tuple[nx.DiGraph, NodeMapping, EdgeMapping]:
    ...
    pos_em, neg_em = (
        dict(),
        dict(),
    )  # Key - edge in reduced graph. Value - list of edges of original graph
...
def reduce_graph2(src_G, x_dict):
    ...
    while True:
        ...
        reducing_stack += [
            dict(op="edges", u=u, v=v, uv_k=k1, vu_k=k2, add_e=edge_to_add)
        ]
