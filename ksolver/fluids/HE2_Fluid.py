from ksolver.hydraulics.properties.Mishenko import Mishenko, from_oil_params
from ksolver.tools.HE2_ABC import HE2_ABC_Fluid, oil_params
import numpy as np
...
class HE2_DummyWater(HE2_ABC_Fluid):
    def __init__(self):
        ...
        self.CurrentLiquidDensity_kg_m3 = 1000
        self.LiquidDensity_SC_kg_m3 = self.CurrentLiquidDensity_kg_m3
        self.mu_wat_cp = 1
        self.viscocity = 1e-3
...
class HE2_BlackOil(HE2_ABC_Fluid):
    def __init__(self, oil_params: oil_params):
        ...
        self.CurrentLiquidDensity_kg_m3 = (
            self.oil_params.oildensity_kg_m3
            * (1 - self.oil_params.volumewater_percent / 100)
            + self.oil_params.waterdensity_kg_m3
            * self.oil_params.volumewater_percent
            / 100
        )
        self.LiquidDensity_SC_kg_m3 = self.CurrentLiquidDensity_kg_m3

    def calc(self, P_bar, T_C, X_kgsec=0, IntDiameter=None):
        P_for_PVT = max(abs(P_bar), 0.75)
        calc_params = self.oil_params

        tubing = {"IntDiameter": IntDiameter} if IntDiameter else None
        temp_mishenko = from_oil_params(
            P_for_PVT, T_C, X_kgsec, calc_params=calc_params, tubing=tubing
        )
        # Side effects
...
def make_fluid_vectors(fluids):
    ...
    owg_mix_pseudo_density_vec = (
        oil_ro_vec * (1 - wc_vec)
        + wat_ro_vec * wc_vec
        + gas_ro_vec * (1 - wc_vec) * gf_vec
    )
...
def dot_product(
    ...
    rez_oil_params = oil_params(
        sat_P, plast_T, gf, oil_ro, wat_ro, gas_ro, oil_Visc, wc * 100, Volume_keff
    )
...
def gimme_dummy_oil_params(volumeWater=50) -> oil_params:
    rez = oil_params(
        sat_P_bar=66.7,
        plastT_C=84,
        gasFactor=39,
        ...
        volumewater_percent=volumeWater,
        volumeoilcoeff=1.015,
    )
    return rez
...
def gimme_BlackOil(op:oil_params, **kwargs):
    d = op._asdict()
    d.update(kwargs)
    fluid = HE2_BlackOil(oil_params(**d))
    return fluid
