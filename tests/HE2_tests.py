import unittest
from ksolver.graph.edges.HE2_Pipe import (
    HE2_WaterPipeSegment,
    HE2_OilPipeSegment,
    HE2_OilPipe,
)
from ksolver.graph.edges.HE2_Pipe import HE2_WaterPipe
import ksolver.uniflocpy.uTools.uconst as uc
import ksolver.hydraulics.methodics.Mukherjee_Brill as mb
import ksolver.hydraulics.properties.Mishenko as msch
...
import numpy as np
import pandas as pd
from ksolver.fluids import HE2_MixFluids as mixer
from ksolver.fluids.HE2_Fluid import HE2_BlackOil
from ksolver.solver import HE2_Fit
from ksolver.tools import HE2_Visualize as vis, HE2_tools as tools
from ksolver.tools.cachespline import create_lazy_spline_cache_f_wrapper
...
class TestWaterPipe(unittest.TestCase):
    ...
    def test_5(self):
        ...
        for x, d, r in conditions:
            ...
            oil_params = dict(
                CurrentP=p0_bar, PlastT=t0_C, CurrentT=t0_C, adkuLiquidDebit=Q_m3day
            )
            oil_params.update(
                wellopVolumeWater=100,
                GasFactor=1e-8,
                PlastWaterWeight=pipe.fluid.rho_wat_kgm3 / 1000,
                wellopLiquidDensityTm=pipe.fluid.rho_wat_kgm3 / 1000,
                adkuOilDebit=0,
            )
            oil_params.update(
                VolumeOilCoeff=1.2,
                PlastOilDynamicViscosity=1,
                SepOilDynamicViscosity=1,
                OilSaturationP=100,
                PlastOilWeight=0.85,
                SepOilWeight=0.85,
                GasDensity=0.8,
            ...
            tubing = dict(
                IntDiameter=pipe.inner_diam_m, angle=90, Roughness=pipe.roughness_m
            )
    ...
    def test_8(self):
        ...
        data = dict(
            dxs=[pipe.dx_m],
            dys=[pipe.uphill_m],
            diams=[pipe.inner_diam_m],
            rghs=[pipe.roughness_m],
        )
        pipeline = HE2_WaterPipe(**data)
...
class TestWaterNet(unittest.TestCase):
    ...
    def test_13(self):
        G, n_dict = tools.generate_random_net_v0(
            randseed=42, N=6, E=8, SNK=1, SRC=2, SEGS=2
        )
        solver = HE2_Solver(G)
        solver.solve()
        shifts = dict(
    ...
    def test_14(self):
        ...
        for rs in range(50):
            G, n_dict = tools.generate_random_net_v0(
                randseed=rs, N=6, E=8, SNK=1, SRC=2, SEGS=2
            )
            solver = HE2_Solver(G)
            try:
            ...
            if resd1 + resd2 > 1e-3:
                self.handle_error(
                    G,
                    rs,
                    f"1stCL residual is {resd1: .2f}, 2ndCL residual is {resd2: .2f}",
                    n_dict,
                )
                errs += [rs]
                continue
    ...
    def test_18(self):
        for rs in range(50):
            ...
            resd1, resd2 = tools.check_solution(G)
            print(
                f"Randseed is {rs}, fun is {op_result.fun: .2f}, 1stCL residual is {resd1: .2f}, 2ndCL residual is {resd2: .2f}",
                n_dict["p_nodes"],
            )
        # tools.draw_solution(G, None, **n_dict)
        # assert op_result.fun < 1e-3, 'Cant solve'
...
class TestFluidMixer(unittest.TestCase):
    ...
    def test_22(self):
        ...
        ethalon = {("KNS_0", "junc_0"): np.array([1.0, 0.0])}
        ethalon.update({("KNS_1", "junc_0"): np.array([0.0, 1.0])})
